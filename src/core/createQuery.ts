...
const createQuery = <R, P extends unknown[]>(
  ...
  const loading = ref((initialState && initialState.loading) || false);
  const data = ref(
    (initialState && initialState.data) || initialData
  ) as Ref<R>;
  const error = ref(initialState && initialState.error);
  const params = ref((initialState && initialState.params) || []) as Ref<P>;
