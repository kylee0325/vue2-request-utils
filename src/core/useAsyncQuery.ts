...
function useAsyncQuery<R, P extends unknown[], FR>(
  ...
  const updateCache = (state: State<R, P>) => {
    ...
    const cacheQueries = (cacheData && cacheData.queries) || {};
    const queryData = unRefObject(state);
    const currentQueryKey = queryKey
      ? queryKey(...state.params.value)
      : QUERY_DEFAULT_KEY;
    ...
    setCache<R, P>(
      cacheKey,
      {
        queries: {
          ...cacheQueries,
          [currentQueryKey]: {
            ...(cacheQueries ? cacheQueries[currentQueryKey] : null),
            ...queryData,
          },
        },
  ...
  const run = (...args: P) => {
    ...
    const newKey = queryKey ? queryKey(...args) : QUERY_DEFAULT_KEY;
  ...
  if (!manual) {
    ...
    // TODO: need refactor
    const cache = getCache<R, P>(cacheKey!);
    const cacheQueries = (cache && cache.data.queries) || {};
